...
import { Container, Grid, Card, CardContent, Typography, TextField, Button, Box, FormControl, InputLabel, Select, MenuItem, Chip, Alert, CircularProgress, IconButton } from '@mui/material'
import { ContentPaste, FormatAlignLeft, Edit, Delete, Close, Visibility } from '@mui/icons-material'
...
export default function Home({ addToast, initialId = '', viewMode = false }) {
  ...
  const [updateValidation, setUpdateValidation] = useState({ valid: false, error: null })
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await client.createJson(jsonContent, password, expiresIn)
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await client.updateJson(endpointData.id, updateJson, updatePassword, updateExpiresIn)
  ...
  const viewRawJson = () => {
    if (endpointData) {
      window.open(`${window.location.origin}/api/json/${endpointData.id}/content`, '_blank')
    }
  }
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                        ...
                        />
                        <IconButton
                          onClick={() => copyToClipboard(`${window.location.origin}/api/json/${endpointData.id}`, 'API URL')}
                        >
                          <ContentPaste />
                        </IconButton>
                      </Box>
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        ...
                        <Typography variant="body2">
                          <strong>Created:</strong> {DateHelper.formatDateTime(endpointData.createdAt)}
                        </Typography>
                        <Typography variant="body2">
                          <strong>Modified:</strong> {DateHelper.formatDateTime(endpointData.modifiedAt)}
                        </Typography>
                        <Typography variant="body2">
                          <strong>Expires:</strong> {DateHelper.formatDateTime(endpointData.expires)}
                        </Typography>
                        <Typography variant="body2">
                          <strong>Status:</strong> 
                          <Chip
                            label={DateHelper.formatRelative(endpointData.expires)}
                            color={DateHelper.formatRelative(endpointData.expires) === 'Expired' ? 'error' : 'success'}
                            size="small"
                            sx={{ ml: 1 }}
                          />
                        </Typography>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                      ...
                      <Button
                        ...
                        disabled={DateHelper.formatRelative(endpointData.expires) === 'Expired'}
                      >
                        Update
                      </Button>
                      <Button
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      <Button
                        variant="outlined"
                        size="small"
                        onClick={() => copyToClipboard(JsonHelper.formatForDisplay(endpointData.json), 'JSON')}
                      >
                ...
                <CardContent>
                  ...
                  <Box component="form" onSubmit={handleUpdate}>
                    <TextField
                      ...
                      error={!updateValidation.valid && updateValidation.error !== null}
                      helperText={updateValidation.error || 'Enter valid JSON data'}
                    ...
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      ...
                      <Button
                        type="submit"
                        variant="contained"
                        disabled={updateLoading || !updateJson.trim() || !updateValidation.valid}
                      >
              ...
              <Card sx={{ mt: 3 }}>
                <CardContent>
                  ...
                  <Alert severity="warning" sx={{ mb: 2 }}>
                    ⚠️ This action cannot be undone. The endpoint will be permanently deleted.
                  </Alert>
              ...
              <CardContent>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                    ...
                    <Button
                      variant="outlined"
                      onClick={() => copyToClipboard(endpoint.endpointUrl, 'API URL')}
                    >
                      <ContentPaste />
                    </Button>
                  </Box>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                    ...
                    <Button
                      variant="outlined"
                      onClick={() => copyToClipboard(endpoint.viewUrl, 'View URL')}
                    >
                      <ContentPaste />
                    </Button>
                  </Box>
                ...
                <Box sx={{ mb: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    ...
                    <Typography variant="body2">
                      <strong>Created:</strong> {new Date(endpoint.createdAt).toLocaleString()}
                    </Typography>
                    <Typography variant="body2">
                      <strong>Expires:</strong> {new Date(endpoint.expires).toLocaleString()}
                    </Typography>
                  </Box>
                </Box>
