...
class MockJGoApp {
  ...
  setupJsonValidation() {
    ...
    updateJsonEditor.addEventListener("input", () => {
      ...
      validateJson(
        updateJsonEditor,
        document.getElementById("updateValidation")
      );
    });
  }
  ...
  async handleCreate() {
    ...
    try {
      ...
      const response = await this.client.createJson(
        jsonContent,
        password,
        expiresIn
      );
  ...
  async handleUpdate() {
    ...
    try {
      ...
      const response = await this.client.updateJson(
        id,
        jsonContent,
        password,
        expiresIn
  ...
  showViewResult(endpoint) {
    ...
    viewJsonContent.textContent = JSON.stringify(
      JSON.parse(endpoint.json),
      null,
      2
    );
  ...
  setLoading(formId, loading) {
    ...
    if (loading) {
      ...
      submitBtn.textContent =
        submitBtn.id === "deleteForm"
          ? "Delete Endpoint"
          : submitBtn.id === "updateForm"
            ? "Update Endpoint"
            : submitBtn.id === "viewForm"
              ? "Load JSON"
              : "Create Endpoint";
